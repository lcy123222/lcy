...
def load_image(root, series):
    ...
    img_file = os.path.join(root, series + ".nii")
    print("path",img_file)
...
def full_dataset(dir, images):
    ...
    image_files = glob(os.path.join(image_path, "*.nii"))
    image_list = []
...
def make_dataset(dir, images, targets, seed, train, class_balance, partition, nonempty, test_fraction, mode):
    ...
    label_files = glob(os.path.join(label_path, "*.nii"))
    label_list = []
...
def normalize_lung_CT(**kwargs):
    ...
    file_list=glob(luna_subset_path + "/" + "*.nii")
...
def normalize_lung_mask(**kwargs):
    ...
    file_list=glob(os.path.join(luna_seg_lungs_path, "*.nii"))
    img_spacing = (vox_spacing, vox_spacing, vox_spacing)
    for img_file in file_list:
        itk_img = sitk.ReadImage(img_file)
        (x_space, y_space, z_space) = itk_img.GetSpacing()
...
def normalize_nodule_mask(**kwargs):
    ...
    file_list=glob(luna_normal_path+"*.nii")
...
if __name__ == "__main__":
    src="E:\project\\vnet.pytorch-master\computed-tomography-images-for-intracranial-hemorrhage-detection-and-segmentation-1.3.1\ct_scans"
    root = os.path.dirname(src)
    images = os.path.basename(src)
    dataset = LUNA16(root=root, images=images, transform=None, split=target_split, mode="infer");
    loader = DataLoader(dataset, batch_size=32, shuffle=False, collate_fn=noop)
